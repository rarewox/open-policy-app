...
import { bill_chat, getBill } from '../../scripts/api/gpt';
...
const BillChat = () => {
  ...
  useEffect(() => {
    if (
      inputText === 'Explain the bill to me' ||
  ...
  const loadBillInformation = async () => {
    try {
      ...
      const response = await getBill({ bill_number: bill });
  ...
  const handleSend = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await bill_chat({
        bill_number: bill,
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center bg-white">
        ...
        <Text className="mt-4 text-gray-600 font-medium">Loading bill...</Text>
      </View>
    );
  }
          ...
          <View className="mb-3">
            <Text
              className={`${
                ...
                showSuggestions ? 'p-2 text-xl' : 'p-2 text-sm'
              } font-bold mb-1 mt-2  w-[90px] text-center bg-gray-100 rounded-full`}
            >
              {bill}
            </Text>
          </View>
            ...
            <View className="space-y-3 mb-1">
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setInputText('Explain the bill to me');
                }}
              >
